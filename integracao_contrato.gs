...
const TABELA_CLIENTES = "Clientes Novos";

const INDEX_RAZAO_SOCIAL = 1;
const INDEX_CNPJ = 2;
const INDEX_ENDERECO = 3;
const INDEX_VALOR_RASTREAMENTO = 4;
const INDEX_VALOR_LOGISTICA = 5;
const INDEX_VALOR_FIXO = 6;
const INDEX_VALOR_CADASTRO = 7;
const INDEX_VALOR_CONSULTA = 8;
...
function generateDefaultContract() {
  ...
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName(TABELA_CLIENTES);
  const rows = sheet.getDataRange().getValues();
  
  rows.forEach(function(row,index) {
    
    if (index === 0) return;                                                                           // Se estiver vazio, ignora
    if (row[9]) return;                                                                                // Se já existir link gerado, ignora.
